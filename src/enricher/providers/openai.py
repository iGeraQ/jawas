...
@register(AIProviderName.OPENAI)
class OpenAIProvider(AIProvider):
    def __init__(self):
        self._client = openai.OpenAI(api_key=settings.openai_api_key)
        self._bucket = TokenBucket(rate=settings.openai_rpm)
    ...
    def score(self, title: str, content: str) -> int:
        try:
            ...
            score = max(0, min(10, int(response.choices[0].message.content.strip())))
            logger.info("item_scored", provider="openai", title=title[:50], score=score)
            return score
        except Exception as e:
            logger.warning("scoring_failed", provider="openai", title=title[:50], error=str(e))
            return 0
    ...
    def synthesize(
        ...
        for network in networks:
            ...
            try:
                ...
                drafts[network] = response.choices[0].message.content.strip()
                logger.info("draft_generated", provider="openai", network=network, title=title[:50])
            except Exception as e:
                logger.error("synthesis_failed", provider="openai", network=network, error=str(e))
        return drafts
